...
- **스케줄러 (주기 자동 수집) — 호출량 예산 반영 필수.** 10분 주기 자동 ingest. 같이 구현할 것: (1) "최근 X분 내 수집했으면 스킵" 가드 — `POST /events/ingest` 연타로 공공 API를 불필요하게 때리는 것 방지, (2) 소스별 폴링 주기 분리 가능하게 — 특히 긴급재난문자는 일일 한도 1,000이라 10분 이상 주기 고정, (3) 실패 재시도는 백오프 포함.
...
*마지막 업데이트: 2026-07-20 (호출량 예산 분석 시점)*
