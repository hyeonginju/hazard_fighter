...
- **JWT 인증.** 지금은 `user_email`로 사용자를 get-or-create하는 임시 방식. 인증 붙으면 인증된 사용자에서 가져오게 교체(`/device-tokens`·`/subscriptions`·`/notifications`의 `user_email` 파라미터 제거).
...
*마지막 업데이트: 2026-07-23 (알림 dedupe — 통보문 시그니처로 분할 구역 합치기)*
