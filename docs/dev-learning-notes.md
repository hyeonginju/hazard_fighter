...
### 2026-07-17 (밤 2) — Phase 2 시작: LLM 알림 문구 생성
...
### 2026-07-17 (밤 3) — LLM 프로바이더 폴백 체인 + 쿨다운
...
- ~~구독 생성 시 소급 평가~~ ✅ 2026-07-17 완료 — backfill_subscription, 실기기 검증됨
- ~~지역명 정규화(최소 버전)~~ ✅ 2026-07-17 완료 — 시/군 접미사 정규화. 행정구역 코드 기반 근본 해결은 여전히 TODO
- **홍수 관측소 동적 선정.** 지금은 청주 2곳 하드코딩 → 구독 지역 기반으로 river_gauges/gauge_region_maps 테이블 활용.
- ~~알림 문구 LLM 생성~~ ✅ 2026-07-17 완료 — fallback 구조 포함, 실데이터 검증 (Part 4 일지 참고)
...
*마지막 업데이트: 2026-07-17 (LLM 폴백 체인 완성 시점)*
