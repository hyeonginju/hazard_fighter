...
Phase 1 완료 + Phase 2 대부분 완료. **실기기까지 end-to-end 실증됨** (2026-07-22): 공공 API 실데이터 → 이름 기반 지역 매칭 → 위험도 판단(규칙+LLM) → LLM 개인화 문구 → FCM v1 실발송 → 모바일 크롬 백그라운드 수신. 테스트 144개 통과.
...
**⚠️ 현재 프로덕션 장애 (2026-08-17 점검에서 발견, 미해결):**
1. **수집 전면 중단** — 8/15 14시 무렵부터 **4개 소스 전부 ConnectTimeout**(07-27 엔 hrfco 하나뿐이었다). 신규 이벤트 0건. 소스별 에러 격리 때문에 앱은 200 을 돌려주고 `/health` 도 정상이라 겉으로는 멀쩡해 보인다. 원인 후보: 공공데이터포털 해외 IP 차단 확대 / Cloud Run egress / 키 만료. **차단이면 서울 리전 이전이 필수가 된다.**
...
0. **[긴급] 수집 전면 중단 원인 규명** — 4개 소스 ConnectTimeout. 로컬에서 같은 키로 호출해 되는지부터 확인(되면 클라우드 IP 문제로 좁혀진다) → 서울 리전 이전 검토.
