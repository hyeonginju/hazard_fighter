...
0. **[긴급] 수집 회복 설계** (원인 규명은 08-28 완료 — 위 1번 참고). 방향 두 가지가 08-23 Neon 문제와 원인을 공유한다: ① **수집 주기 늘리기** — 10분 크론이 Neon 을 못 자게 했고 인스턴스를 계속 살려둬 나쁜 IP 에 가두기도 했다. ② **자가복구** — 전 소스가 N 사이클 연속 실패하면 프로세스를 종료해 인스턴스·IP 를 다시 뽑게 한다(측정된 성공률 약 1/3이라 몇 번이면 걸린다). 리전 이전은 **하지 말 것**(측정으로 배제됨).
