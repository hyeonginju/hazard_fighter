...
Phase 1 완료 + Phase 2 대부분 완료. 실데이터로 "공공 API → 지역 매칭 → 위험도 판단(규칙+LLM) → 개인 맞춤 알림 생성"까지 end-to-end 검증됨. 테스트 47개 통과.

**동작하는 것:** 공공 API 3종 연동(기상특보/지진/홍수통제소), 특보 t6 파싱 지역 매칭, Layer 1 규칙 매트릭스, Layer 2 LLM 보조 판단(+ai_risk_logs), 구독 소급 평가, LLM 문구 생성+3단계 폴백 체인, 10분 주기 스케줄러+중복 가드, 로그 시크릿 마스킹.

**다음 할 일 (우선순위):**
1. **긴급재난문자 ingestion 클라이언트** — 키·스펙 확인 완료(safetydata `DSSP-IF-00247`). `MSG_CN`(내용)/`RCPTN_RGN_NM`(지역)/`CRT_DT`(시각)/`DST_SE_NM`(재난구분)/`SN`(일련번호=dedupe 키). **일일 한도 1,000건**이라 주의. 날짜 필터 파라미터 확인 필요. `app/ingestion/` 기존 클라이언트 패턴(`base.py` 상속) 따를 것.
2. **홍수통제소 호출 스로틀링** — 관측소 동적 선정 도입 시 분당 1,000건 초과하면 키 차단(3회 위반). 순차 호출/스로틀 필요.
3. FCM 웹푸시 발송(서비스 계정 JSON), 웹 PWA 구독 화면, JWT 인증.
